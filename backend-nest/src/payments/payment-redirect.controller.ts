...
const APP_SCHEME = (process.env.APP_DEEP_LINK_SCHEME ?? 'malahm').replace(
  /:\/\/$/,
  '',
);
const ANDROID_PACKAGE = process.env.APP_ANDROID_PACKAGE ?? 'com.sarh.butcher';
...
<body>
  <div class="card">
    ...
    <h1>${title}</h1>
    <p>${message}</p>
    <a class="btn" id="open" href="${deepLink}">فتح تطبيق ملاحم سرح</a>
